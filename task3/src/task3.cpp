...
struct neworder{
	...
	std::vector<unsigned> primaryKey;
	map_t m;
	std::multimap<std::tuple<Integer,Integer>, Integer> oindex;
	void init(){
		primaryKey.push_back(2);
		primaryKey.push_back(1);
		primaryKey.push_back(0);
		...
		while(!f.eof() && f >> line){
			...
			oindex.insert(std::make_pair(std::make_tuple(row.no_w_id,row.no_d_id),row.no_o_id));
		}
	}
	Integer getMini(Integer no_w_id, Integer no_d_id){
		std::multimap<std::tuple<Integer,Integer>,Integer>::iterator it = oindex.find(std::make_tuple(no_w_id, no_d_id));
		if(it != oindex.end())
			return it->second;
		return -1;
	}
...
void delivery(int32_t w_id, int32_t o_carrier_id, Timestamp datetime, Norder::order &order, Nneworder::neworder &neworder, Norderline::orderline &orderline, Ncustomer::customer &customer){
	for(int d_id =1; d_id <=10; d_id++){
		Integer o_id = -1;
		o_id = neworder.getMini(w_id,d_id);
		
		if(o_id == -1)
			continue;
		Nneworder::neworder::Row *neworderrow = neworder.find(std::make_tuple(Integer(w_id), Integer(d_id), o_id));
	    	
		neworder.remove(*neworderrow);

		Norder::order::Row *orderrow = order.find(std::make_tuple(Integer(w_id),Integer(d_id),o_id));
...
double query(Norder::order &order, Ncustomer::customer &customer, Norderline::orderline &orderline){
	std::list<Norder::order::Row> orders = order.getAll();
	std::list<Ncustomer::customer::Row> customers = customer.getAll();
	std::list<Norderline::orderline::Row> orderlines = orderline.getAll();

	std::list<orderkey> orderkeys;
	...
	typedef std::unordered_map<const key_customer ,Ncustomer::customer::Row,key_hash_customer, key_equal_customer> map_customer;
	...
	for(Ncustomer::customer::Row c_ustomer : customers){
		if(c_ustomer.c_last.value[0] != 'B')
			continue;
		m_customer[std::make_tuple(c_ustomer.c_w_id, c_ustomer.c_d_id, c_ustomer.c_id)] = c_ustomer;
	}
	...
	for(Norder::order::Row o_rder : orders){
		...
		if(itr != m_customer.end()){
			...
			key.c_balance = itr->second.c_balance;
	...
	std::multimap<key_orderline, Norderline::orderline::Row> m_orderline;

	for(Norderline::orderline::Row o_rderline : orderlines){
		m_orderline.insert(std::make_pair(std::make_tuple(o_rderline.ol_o_id, o_rderline.ol_d_id, o_rderline.ol_w_id), o_rderline));
	}

	double sum = 0;
	for(orderkey key : orderkeys){
		auto itr = m_orderline.find(std::make_tuple(key.o_id, key.o_d_id,key.o_w_id));
		while(itr != m_orderline.end()){
			sum += (itr->second.ol_quantity.value * itr->second.ol_amount.value - key.c_balance.value * key.o_ol_cnt.value);
			itr++;
		}
	}
...
void transaction(Nwarehouse::warehouse &warehouse, Ncustomer::customer &customer, Ndistrict::district &district, Nstock::stock &stock, Nitem::item &item, Norder::order &order, Nneworder::neworder &neworder, Norderline::orderline &orderline){
	int rnd=urand(1,100);
	if (rnd<=10) {
...
//	}
	...
	int querynumber = 0;
	childRunning=true;
	std::clock_t start_fork=std::clock();
	double duration_fork;
	pid_t pid=fork();
	duration_fork = (std::clock()-start_fork) / (double) CLOCKS_PER_SEC;

	if(pid){ // execute query in first level parent
		query(order, customer, orderline);
		querynumber++;
		while (childRunning){
			pid_t ppid = fork(); // create later level parent and child
			if(ppid){
				query(order, customer, orderline); // execute query no matter parent or child
				querynumber++;
			}else{
				query(order, customer, orderline); // execute query no matter parent or child
				return 0;
	...
	}else { // execute transaction in first level child
		for(int i = 0; i < 1000000; i++){
			transaction(warehouse, customer, district, stock, item, order, neworder, orderline);
		}
		return 0;
   }
	...
	std::cout << "number of transactions per second is " << 1000000 / duration << std::endl;
	std::cout << "the average duration of query is " << duration / querynumber << std::endl;
	std::cout << "the fork takes " << duration_fork << std::endl;
