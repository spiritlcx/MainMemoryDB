...
// Manually built algebra tree


//	TableScan* tableScan = new TableScan(customers);	
...
//	TableScan* tableScanB = new TableScan(orders);
...
//	TableScan* tableScanC = new TableScan(orderlines);
...
typedef std::tuple<Integer,Integer,Integer> key_tJoinA;
struct key_hashJoinA : public std::unary_function<key_tJoinA, std::size_t>{
	std::size_t operator()(const key_tJoinA& k) const{
		return std::get<0>(k).value+std::get<1>(k).value+std::get<2>(k).value;
	}
};

struct key_equalJoinA : public std::binary_function<key_tJoinA, key_tJoinA, bool>{
	bool operator()(const key_tJoinA& v0, const key_tJoinA& v1) const{
		return (
...
std::vector<JoinA> JoinAs;

std::unordered_multimap<key_tJoinA, int, key_hashJoinA,key_equalJoinA> hashmapJoinA;
typedef std::tuple<Integer,Integer,Integer> key_tJoin;
struct key_hashJoin : public std::unary_function<key_tJoin, std::size_t>{
	std::size_t operator()(const key_tJoin& k) const{
		return std::get<0>(k).value+std::get<1>(k).value+std::get<2>(k).value;
	}
};

struct key_equalJoin : public std::binary_function<key_tJoin, key_tJoin, bool>{
	bool operator()(const key_tJoin& v0, const key_tJoin& v1) const{
		return (
			std::get<0>(v0) == std::get<0>(v1)&&
			std::get<1>(v0) == std::get<1>(v1)&&
			std::get<2>(v0) == std::get<2>(v1)
...
std::unordered_multimap<key_tJoin, int, key_hashJoin,key_equalJoin> hashmapJoin;
for(unsigned i = 0; i < customer.size(); i++){
	if(customer[i].c_id==322 && customer[i].c_w_id==1 && customer[i].c_d_id==1){
		hashmapJoin.insert(std::make_pair(std::make_tuple(customer[i].c_w_id,customer[i].c_d_id,customer[i].c_id), i));
	}
}
for(unsigned i = 0; i < order.size(); i++){
	int temp = i;
	int count = 0;
	auto range = hashmapJoin.equal_range(std::make_tuple(order[i].o_w_id,order[i].o_d_id,order[i].o_c_id));
	for(auto it = range.first; it != range.second; it++){
		Join t;
		t.c_first = customer[it->second].c_first;
		t.c_last = customer[it->second].c_last;
		t.o_all_local = order[i].o_all_local;
		t.o_w_id = order[i].o_w_id;
		t.o_d_id = order[i].o_d_id;
		t.o_id = order[i].o_id;
		Joins.push_back(t);
		i = count;
		count++;
	hashmapJoinA.insert(std::make_pair(std::make_tuple(Joins[i].o_w_id,Joins[i].o_d_id,Joins[i].o_id), i));
	}
	i = temp;
}
for(unsigned i = 0; i < orderline.size(); i++){
	int temp = i;
	int count = 0;
	auto range = hashmapJoinA.equal_range(std::make_tuple(orderline[i].ol_w_id,orderline[i].ol_d_id,orderline[i].ol_o_id));
	for(auto it = range.first; it != range.second; it++){
		JoinA t;
		t.c_first = Joins[it->second].c_first;
		t.c_last = Joins[it->second].c_last;
		t.o_all_local = Joins[it->second].o_all_local;
		t.ol_amount = orderline[i].ol_amount;
		JoinAs.push_back(t);
		i = count;
		count++;
	std::cout << JoinAs[i].c_first <<", " <<JoinAs[i].c_last <<", " <<JoinAs[i].o_all_local <<", " <<JoinAs[i].ol_amount<< std::endl;
	}
	i = temp;
}
