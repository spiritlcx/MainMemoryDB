...
void initialize(){
	...
	out << "#include \"tbb/tbb.h\"" << std::endl;
	...
	out << "tbb::mutex printMutex;" << std::endl;
}
...
void TableScan::produce(){
	out << "tbb::parallel_for(tbb::blocked_range<unsigned>(0,"+tablename+"->size()),[&](const tbb::blocked_range<unsigned>& _range){" << std::endl;
	out << "for(unsigned i = _range.begin(); i != _range.end(); i++){" << std::endl;
	out << "\t";
	consumer->consume();
	out << "}" << std::endl;
	out << "});" << std::endl;
...
void Print::consume(){
	...
	out << "\tprintMutex.unlock();" << std::endl;
}
...
void HashJoin::produce(){
	...
	out << ", key_tvalue"+joinname+", key_hash"+joinname+",key_equal"+joinname+"> hashmap"+joinname+";"<<std::endl;
	...
	if(fromleft){
		...
		for(std::string cname : cnames){	
			...
			if(flag){
				...
				out << "std::get<"+std::to_string(index)+">(it->second);" << std::endl;
				index++;
			}
		}
