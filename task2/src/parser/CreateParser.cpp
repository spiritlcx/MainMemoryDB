   ...
   switch(state) {
      ...
      case State::IndexOn:
    	  if (isIdentifier(tok) || isTableName(tok)) {
              ...
              if(currentrel == nullptr){
                  throw ParserError(line, "Table "+token+" doesn't exist");
              }
      ...
      case State::AttributeTypeVarchar:
          if (tok.size()==1 && tok[0]==literal::ParenthesisLeft)
             ...
             throw ParserError(line, "Expected '(' after 'VCHAR', found'"+token+"'");
      ...
      case State::VarcharValue:
         if (tok.size()==1 && tok[0]==literal::ParenthesisRight)
            ...
            throw ParserError(line, "Expected ')' after length of VCHAR, found'"+token+"'");
         break;
      ...
      case State::Null:
         ...
         break;
      default:
         throw ParserError(line, tok + "is not a valid symbol");
   }
}

std::vector<std::string> Parser::split(std::string s, char delimit){
